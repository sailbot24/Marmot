...
#' Calculate Canopy Height Statistics for AOI
#' 
#' Calculates vegetation height statistics from CHM data within an AOI
#' 
#' @param chm_path Path to the CHM raster file
#' @param aoi sf or sfc object representing the area of interest
#' @param height_breaks Numeric vector of height breaks for classification (in meters)
#' @return Named list containing:
#' \itemize{
#'   \item basic_stats: mean, max, min, sd of canopy heights
#'   \item height_distribution: percentage of area in each height class
#'   \item canopy_cover: percentage of area with vegetation above 2m
#' }
#' @export
#' @importFrom terra rast extract
#' @importFrom sf st_transform
calculate_chm_stats <- function(chm_path, aoi, 
                                height_breaks = c(0, 2, 5, 10, 20, 30, 50)) {
  # Read CHM raster
  chm <- terra::rast(chm_path)
  
  # Transform AOI to raster CRS if needed
  if (sf::st_crs(aoi) != terra::crs(chm)) {
    aoi <- sf::st_transform(aoi, terra::crs(chm))
  }
  
  # Extract values within AOI
  values <- terra::extract(chm, terra::vect(aoi))[[1]]
  
  # Basic statistics
  basic_stats <- list(
    mean_height = mean(values, na.rm = TRUE),
    max_height = max(values, na.rm = TRUE),
    min_height = min(values, na.rm = TRUE),
    sd_height = sd(values, na.rm = TRUE)
  )
  
  # Height distribution
  height_dist <- table(cut(values, breaks = height_breaks)) / length(values) * 100
  
  # Canopy cover (% area with vegetation > 2m)
  canopy_cover <- sum(values > 2, na.rm = TRUE) / length(values) * 100
...
#' Plot CHM Height Distribution
#' 
#' Creates a histogram of canopy heights from CHM data
#' 
#' @param chm_path Path to the CHM raster file
#' @param aoi sf or sfc object representing the area of interest
...
plot_chm_distribution <- function(chm_path, aoi, bins = 30) {
  ...
  df <- data.frame(height = values)
  
  # Create plot
  p <- ggplot2::ggplot(df, ggplot2::aes(x = height)) +
    ggplot2::geom_histogram(bins = bins, fill = "forestgreen", color = "white") +
    ggplot2::labs(
      title = "Distribution of Canopy Heights",
      x = "Height (meters)",
      y = "Count"
    ) +
