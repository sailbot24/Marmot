...
process_hexagon <- function(hexagon, id, save_dir = "output") {
  ...
  tryCatch({
    ...
    # Create a simple raster with square cells and random tree heights between 10 and 100 feet
    # Using a fixed extent in projected coordinates for demonstration
    r <- rast(xmin = 0, xmax = 1000, ymin = 0, ymax = 1000, 
              resolution = 10, crs = "EPSG:3857")
    values(r) <- runif(ncell(r), 10, 100)
    ...
    ttops <- vwf(r, winFun = lin, minHeight = 10)
