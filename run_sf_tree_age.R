...
process_hexagon <- function(hexagon, id, save_dir = "output") {
  ...
  tryCatch({
    # Convert sf to sp for the download_oregon_chm function
    hexagon_sp <- as(hexagon, "Spatial")
    
    # Download CHM data
    chm <- download_oregon_chm(hexagon_sp, output_path = chm_file, mask = TRUE, quiet = TRUE)
...
run_tree_age_analysis <- function() {
  ...
  # Create hexagon grid
  hex_grid <- st_make_grid(siuslaw, cellsize = hex_size, square = FALSE)
  ...
  hex_grid_clipped <- st_intersection(hex_grid_sf, st_union(siuslaw))
